...
ABI_FILE_PATH = "out/{}.sol/{}.json"


def export_abi(_contract_name):
    src = ABI_FILE_PATH.format(_contract_name, _contract_name)
...
def extract_last_deployment_address(_contract_name, _chain_id):
    deployment_file_path = DEPLOYMENT_FILE_PATH.format(_contract_name, _chain_id)
...
def export_config():
    ...
    for chain_id in get_chain_ids():
        contract_address = extract_last_deployment_address(MAIN_CONTRACT_NAME, chain_id)
        rpc_url = extract_last_rpc_url(MAIN_CONTRACT_NAME, chain_id)

        config[chain_id] = {"contract_address": contract_address, "rpc_url": rpc_url}
